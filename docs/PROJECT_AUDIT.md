...
### 2.1 Architecture
* **Single Responsibility Principle (SRP):** Every module has a single clear concern. Configurations reside in [config.py](src/config.py), model registry acts as a factory in [registry.py](src/models/registry.py), dataset splitting and Loader initialization reside in [datasets.py](src/models/datasets.py), training routines reside in [trainer.py](src/models/trainer.py), score calculations are in [metrics.py](src/models/metrics.py), plotting is in [plots.py](src/visualization/plots.py), and orchestration is driven by the central [Experiment](src/experiment/experiment.py) class.
